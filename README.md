...
| M5 | 템플릿 관리 · 폼 빌더 · 검증 (드라이런은 v0.5 제거) | 완료 |
| M6 | 가이드 매핑 엔진 (2트랙 · 판정) | 완료 |
| M7 | 보고서 생성 (자체 완결형 HTML) | 완료 |
| M9 | LLM 조치 가이드 (기본 비활성 · 보고서 4절에 선택 첨부) | 완료 |
| M10 | 데스크톱 패키징 (원클릭 빌드 · Tauri 번들) | 완료 |

화면 8종(대시보드 · 템플릿 · 처리 로그 · 스캔 · 탐지 결과 · 보고서 · 조치 가이드 · 설정) 전부 동작
...
**템플릿이 0개면 스캔이 거부됨.** nuclei 가 아무것도 실행하지 않고 정상 종료하므로
0건을 양호로 오독하게 됨. `템플릿` 화면에서 공식 템플릿을 갱신하거나 직접 작성.

스캔 대상은 `스캔` 화면에 그대로 입력. 포트를 붙이지 않으면 80·443 만 검사하며,
`localhost:8000-8100` 처럼 범위 입력도 가능.
...
1. `스캔` 에서 대상·선별 방식·옵션 지정
2. `스캔 시작` — 진행률·탐지 결과·처리 로그가 실시간으로 표시됨
3. `탐지 결과` 에서 필터·상세 확인, 오탐 표시

선별 방식은 네 가지를 지원.

| 방식 | 내용 |
|---|---|
| 전체 검사 | 보유 템플릿 전부 실행 (기본값). 취약점 판정이 없는 WordPress 전수 열거 2개만 제외 |
| 환경 기반 선별 | 제품 식별 후 대상에 없는 제품 전용 템플릿 제외. 애매하면 전체 실행으로 후퇴 |
| 조건 필터 | 태그·심각도로 템플릿 선별 |
| 직접 지정 | 템플릿 ID 를 직접 입력 |

전체 검사만으로 nuclei 기본 실행 대비 요청 76% 감소 (탐지 대상 동일). 전수 열거가 필요하면
설정 → 스캔 옵션에서 켬. 무엇을 실행·제외했는지는 결과 화면과 보고서 A-2 에 기록.

### B. 다른 기기에서 실행한 결과 가져오기

REDAR 를 설치할 수 없는 서버·랩 환경에서는 nuclei 만 실행하고 결과 파일을 옮기면 됨
...
`--nuclei-version` 은 재현성 기록용이며 생략 가능. 화면에는 `외부 임포트` 로 표시됨.
...
| 템플릿 | 인벤토리·폼 빌더·검증 |
| 처리 로그 | nuclei 출력·내부 처리 과정 실시간 확인, 로그 저장 |
| 스캔 | 대상·선별 방식 지정, 실시간 진행률·남은 시간·탐지 피드. 실행 옵션은 설정값 사용 |
| 탐지 결과 | 집계, 필터 4종, 상세(요청·응답·재현 명령·오탐 판정) |
| 보고서 | 생성·미리보기·HTML 내려받기. 생성 시 LLM 조치 상세 가이드(4절) 동시 첨부 선택 (조치 가이드 기능 켠 경우) |
| 조치 가이드 | 보고서 기반 프롬프트 생성, LLM 대화, 보고서 첨부 (기본 비활성) |
| 설정 | 오프라인 모드, 외부 통신 4토글, 스캔 옵션(동시 실행·응답 대기·재시도·초당 상한·전수 열거), LLM |

메뉴는 사용 순서대로 배치. 스캔 전에 템플릿이 있어야 하고, 진행 상황은 처리 로그와 하단 도크로 확인.
하단 도크의 **로그 보기** 는 처리 로그 화면으로 이동하며, 저장은 처리 로그 화면에서만 가능.

화면 상단 상태 띠는 **백엔드 · nuclei · 가이드 DB · 템플릿 · 외부 통신** 을 항상 표시.
다섯 항목 모두 도구의 동작 가능 범위를 바꾸는 값.

심각도 5종과 취약점 유형 14종은 **탐지 0건이어도 항상 전부 표시됨.**
원격 스캐너가 점검할 수 있는 범위는 제한적이며, 점검하지 않은 항목이 양호로 읽히면 안 됨
...
| LLM API | 비활성 | 조치 가이드 생성 (선택). 보고서 본문 1~3절은 미사용 |
...
실측 기준 `nuclei-templates` @ `2d5f9b5` (2026-08-26). 템플릿은 계속 갱신되므로 그 시점 스냅샷
