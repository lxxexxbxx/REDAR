...
필수 구성요소가 아니다 (절대규칙 2). 기본 Provider 가 NullProvider 인 이유
"""
...
# 판정·구조·조치방법 생성에 개입하지 않는다. 산문 필드만 채운다 (절대규칙 2)
_NO_FINDING = (
...
def remediation_summary(item: dict[str, Any]) -> str:
    ...
    원문이 있으면 그대로 인용한다. 다듬은 문장으로 대체 금지 (절대규칙 9)
    """
    return (
...
# 패치 목표 버전이 없는 항목. 빈칸은 검토자에게 데이터 누락으로 읽힌다 (docs/04 A-6)
