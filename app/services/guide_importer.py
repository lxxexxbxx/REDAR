...
본문은 저작권 대상이라 저장소에 없다. 사용자가 파일을 받아 직접 넣는다 (절대규칙 8).
전체 삭제 후 재적재하며 finding_guide_refs 는 건드리지 않는다 - 별도 층이고
본문 없이도 매핑 결과가 유지되어야 한다
"""
...
# case_text 는 항목당 최대 8,675자. 기본 한도(131,072)로 충분하나 여유를 둔다
...
def import_text(
    ...
    """CSV 문자열 임포트. 업로드·CLI 양쪽이 이 경로를 쓴다"""
