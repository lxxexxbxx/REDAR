            ...
            "fields": [
                {
                    ...
                    "type": "string",
                    "required": True,
                    "pattern": TEMPLATE_ID_RE.pattern,
                    "help": "소문자·숫자·하이픈만",
                },
...
def _matchers(raw: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(raw):
        ...
        if kind not in MATCHER_TYPES:
            raise BuildError(
                f"matchers[{index}].type", "허용되지 않은 조건 종류입니다."
            )
        ...
        if kind == "status":
            try:
                ...
                raise BuildError(
                    f"matchers[{index}].values", "status 는 숫자 필요"
                ) from exc
        else:
            part = str(item.get("part") or "body")
            if part not in MATCHER_PARTS:
                raise BuildError(
                    f"matchers[{index}].part", "검사 대상 값이 올바르지 않습니다."
                )
            matcher["part"] = part
        ...
        if item_condition:
            if item_condition not in CONDITIONS:
                raise BuildError(
                    f"matchers[{index}].condition", "and 또는 or 만 쓸 수 있습니다."
                )
