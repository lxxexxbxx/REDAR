...
LOOSE_MATCHER = {
    "code": "LOOSE_MATCHER",
    "message": "matcher 가 status 단독입니다. 정상 페이지에도 매칭되어 오탐이 발생할 수 있습니다.",
    "suggestion": "응답 본문의 고유 문자열을 word matcher 로 추가하세요.",
}
...
def check_policy(yaml_text: str) -> dict[str, Any]:
    ...
    if not template_id:
        errors.append({"field": "id", "message": "템플릿 ID 가 없습니다."})
    elif not builder.TEMPLATE_ID_RE.match(template_id):
        errors.append({
            "field": "id",
            "message": "템플릿 ID 는 소문자·숫자·하이픈만 가능합니다.",
        })
    ...
    if not str(info.get("name") or "").strip():
        errors.append({"field": "info.name", "message": "이름이 없습니다."})

    severity = str(info.get("severity") or "").strip()
    if not severity:
        errors.append({"field": "info.severity", "message": "severity 값이 없습니다."})
    elif severity not in {s.value for s in Severity}:
        errors.append({
            "field": "info.severity",
            "message": f"severity 값이 유효하지 않습니다: {severity}",
        })
    ...
    for index, entry in enumerate(requests):
        ...
        if not entry.get("path"):
            errors.append({"field": f"http[{index}].path", "message": "경로가 없습니다."})
