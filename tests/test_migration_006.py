...
def _v05_db(path) -> None:
    ...
    raw.execute(
        "INSERT INTO scan_targets (scan_id, raw, host)"
...
def test_migration_006_allows_full_scan_and_keeps_children(tmp_path):
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM scan_targets WHERE scan_id = 'scn_old'"
        ).fetchone()[0]
        == 1
    )
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM findings WHERE scan_id = 'scn_old'"
        ).fetchone()[0]
        == 1
    )
    ...
    assert (
        conn.execute(
            "SELECT cnt_total FROM v_scan_summary WHERE scan_id = 'scn_old'"
        ).fetchone()[0]
        == 1
    )
