...
def validate(data_dir: Path, guide_db: Path | None) -> None:
    ...
    unknown_vt = {
        r["vuln_type"]
        for r in conn.execute("SELECT DISTINCT vuln_type FROM vuln_type_rules")
    ...
    if guide_db:
        codes = {
            r[0] for r in sqlite3.connect(guide_db).execute("SELECT code FROM item")
        }
        missing = {
            r["item_code"]
            for r in conn.execute("SELECT DISTINCT item_code FROM guide_mappings")
        } - codes
        if missing:
...
def authored_cwe_codes(data_dir: Path) -> set[str]:
    ...
    with (data_dir / "guide_mappings.csv").open(encoding="utf-8-sig", newline="") as fh:
        return {
            r["match_value"] for r in csv.DictReader(fh) if r["match_type"] == "cwe_id"
        }
...
def generate_advisories(
    con: sqlite3.Connection, out: Path, templates: str | None
) -> None:
...
def generate_template_exceptions(
    ...
    keyword_rules = yaml.safe_load(rules_path.read_text(encoding="utf-8")).get(
        "keyword", []
    )
    rows: list[list] = []
    seen: set[tuple] = set()
    for t in con.execute("SELECT * FROM wp_template WHERE is_detect=0"):
        ...
        hit = next(
            (r for r in keyword_rules if any(k in blob for k in r["match"])), None
        )
        if not hit:
            continue
        key = (t["id"], hit["code"])
        ...
        conf = (
            "high"
            if hit["conf"] >= 0.9
            else ("medium" if hit["conf"] >= 0.6 else "low")
        )
        rows.append(
            [
                "template_id",
...
def main() -> None:
    ...
    ap.add_argument(
        "--templates", help="nuclei-templates 경로. 테마/플러그인 판별에 필요"
    )
