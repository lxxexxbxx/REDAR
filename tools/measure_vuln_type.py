...
def is_detection(t):
    return any(p in t["path"] for p in DETECT_PATHS) or any(
        k in t["id"] for k in DETECT_ID
    )


def load_rules(path):
    rows = [
        dict(r, priority=int(r["priority"]))
            ...
            out.append(
                {
                    ...
                    "cwe": sorted(
                        {m.upper() for x in cwe for m in CWE_RE.findall(str(x))}
                    ),
                }
            )
    return out
...
def resolve(t, rules):
    for r in rules:
        ...
        if r["match_type"] == "template_prefix" and t["id"].startswith(
            r["match_value"]
        ):
